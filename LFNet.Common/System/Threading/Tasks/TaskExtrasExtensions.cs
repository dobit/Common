    ...
    public static class TaskExtrasExtensions
    {
        ...
        private class TaskObservable<TResult> : IObservable<TResult>
        {
            ...
            public IDisposable Subscribe(IObserver<TResult> observer)
            {
                ...
                this._task.ContinueWith(delegate(Task<TResult> t)
                {
                    switch (t.Status)
                    {
                        ...
                        case TaskStatus.Faulted:
                            observer.OnError(this._task.Exception);
                            return;
                        default:
                            return;
                    }
                }, cancellationTokenSource.Token);
                return new TaskExtrasExtensions.CancelOnDispose
                {
        ...
        public static IObservable<TResult> ToObservable<TResult>(this Task<TResult> task)
        {
            ...
            return new TaskExtrasExtensions.TaskObservable<TResult>
            {
                _task = task
            };
        }
        ...
        public static void WaitWithPumping(this Task task)
        {
            ...
            task.ContinueWith<bool>((Task _) => nestedFrame.Continue = false);
        ...
        public static Task Then(this Task task, Func<Task> next)
        {
            ...
            task.ContinueWith(delegate
            {
                ...
                try
                {
                    next().ContinueWith<bool>((Task t) => tcs.TrySetFromTask(t), TaskScheduler.Default);
                }
                catch (Exception exception)
                {
                    tcs.TrySetException(exception);
        ...
        public static Task Then<T>(this Task<T> task, Func<T, Task> next)
        {
            ...
            task.ContinueWith(delegate
            {
                ...
                try
                {
                    next(task.Result).ContinueWith<bool>((Task t) => tcs.TrySetFromTask(t), TaskScheduler.Default);
                }
                catch (Exception exception)
                {
                    tcs.TrySetException(exception);
        ...
        public static Task<TResult> Then<TResult>(this Task task, Func<Task<TResult>> next)
        {
            ...
            task.ContinueWith(delegate
            {
                ...
                try
                {
                    next().ContinueWith<bool>((Task<TResult> t) => tcs.TrySetFromTask(t), TaskScheduler.Default);
                }
                catch (Exception exception)
                {
                    tcs.TrySetException(exception);
        ...
        public static Task<TNewResult> Then<TResult, TNewResult>(this Task<TResult> task, Func<TResult, Task<TNewResult>> next)
        {
            ...
            task.ContinueWith(delegate
            {
                ...
                try
                {
                    next(task.Result).ContinueWith<bool>((Task<TNewResult> t) => tcs.TrySetFromTask(t), TaskScheduler.Default);
                }
                catch (Exception exception)
                {
                    tcs.TrySetException(exception);
